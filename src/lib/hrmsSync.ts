 ...
 */
/** Update journey status manually for an employee. */
export function setStatus(employeeId: string, status: "in_progress" | "completed", stepTitle?: string, stepId?: string): void {
  if (typeof window === "undefined") return;
  try {
    const key = `employeeJourneyStatus_${employeeId}`;
    const raw = localStorage.getItem(key);
    ...
    if (status === "completed") {
      ...
      if (newStatus === "Resigned") {
        empTriggers.push({
          ...
          description: "Employee resigned; NACH and account eligibility rules applied.",
          date: syncedAt,
          source: "MMFSL LMS",
        });
        ...
        empTriggers.push({
          type: "NACH_RESIGNATION",
          label: "NACH – Resignation",
          description: "Employee on notice; NACH and account eligibility rules applied.",
          date: syncedAt,
          source: "MMFSL LMS",
        });
        ...
        empTriggers.push({
          ...
          description: "Employment terminated; NACH stop and account eligibility rules applied.",
          date: syncedAt,
          source: "MMFSL LMS",
        });
      }
