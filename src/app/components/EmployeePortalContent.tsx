...
import {
    ...
    openEmployeeLoanJourneyInNewTab,
...
export default function EmployeePortalContent({ activePage, onNavigate, currentEmployeeId, currentEmployee, onResumeFromNudge, onStartSalaryJourney, onContinueHubJourney }: EmployeePortalContentProps) {
        ...
        return (
            <div className="space-y-6">
                ...
                <div className="pt-12 pb-6 border-t border-[#E5E7EB] flex flex-col md:flex-row items-center justify-between gap-4">
                    <p className="text-xs text-[#6B7280]">© 2026 State Bank of India. All rights reserved.</p>
    ...
    if (activePage === "finagent") {
        const pref = currentEmployee
            ? {
                employeeId: currentEmployee.id,
                name: currentEmployee.name,
                email: currentEmployee.email,
                phone: currentEmployee.phone,
                mobileNumber: currentEmployee.phone,
            }
            : undefined;
        ...
        return (
            <div className="max-w-2xl">
                ...
                <div className="rounded-xl border border-[#E8EAED] bg-white p-8 space-y-6">
                    <div className="text-center">
                        <PiggyBank className="w-12 h-12 text-dashboard-primary mx-auto mb-4" />
                        <p className="text-[#6B7280] mb-2">Get instant answers about your salary account, offers, and more.</p>
                    </div>
                    ...
                    <div className="space-y-4 pt-2 border-t border-[#E8EAED]">
                        <div className="rounded-lg border border-[#E8EAED] bg-[#FAFBFC] p-5 text-center">
                            ...
                            <button
                                ...
                                className="h-12 px-6 bg-dashboard-primary text-white font-semibold rounded-xl hover:opacity-90 w-full sm:w-auto"
                            >
                                Apply for Personal Loan
                            </button>
                        </div>
                        <div className="rounded-lg border border-[#E8EAED] bg-[#FAFBFC] p-5 text-center">
                            <Car className="w-10 h-10 text-dashboard-primary mx-auto mb-2" />
                            <p className="text-sm font-semibold text-[#111827] mb-1">Auto Loan</p>
                            <p className="text-sm text-[#6B7280] mb-4">Finance a new or pre-owned vehicle with structured auto loan options (demo flow).</p>
                            <button
                                type="button"
                                onClick={() => (pref ? openEmployeeLoanJourneyInNewTab("auto-loan", pref) : openEmployeeLoanJourneyInNewTab("auto-loan"))}
                                className="h-12 px-6 bg-dashboard-primary text-white font-semibold rounded-xl hover:opacity-90 w-full sm:w-auto"
                            >
                                Apply for Auto Loan
                            </button>
                        ...
                        <div className="rounded-lg border border-[#E8EAED] bg-[#FAFBFC] p-5 text-center">
                            <GraduationCap className="w-10 h-10 text-dashboard-primary mx-auto mb-2" />
                            <p className="text-sm font-semibold text-[#111827] mb-1">Education Loan</p>
                            <p className="text-sm text-[#6B7280] mb-4">Support higher education expenses with education loan journey (demo flow).</p>
                            <button
                                type="button"
                                onClick={() => (pref ? openEmployeeLoanJourneyInNewTab("education-loan", pref) : openEmployeeLoanJourneyInNewTab("education-loan"))}
                                className="h-12 px-6 bg-dashboard-primary text-white font-semibold rounded-xl hover:opacity-90 w-full sm:w-auto"
                            >
                    ...
                    <div className="pt-6 border-t border-[#E8EAED] flex items-center justify-center gap-6">
                        <button type="button" onClick={() => setShowTerms(true)} className="text-xs text-[#6B7280] hover:text-dashboard-primary transition-colors">Terms & Conditions</button>
                        <button type="button" onClick={() => setShowPrivacy(true)} className="text-xs text-[#6B7280] hover:text-dashboard-primary transition-colors">Privacy Policy</button>
                    </div>
                </div>
            </div>
        );
