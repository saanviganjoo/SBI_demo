...
/** Aadhaar e-KYC: Only Aadhaar number + UIDAI consent. DOB/PAN captured on Page 1 (welcome step). */
export default function StepEkycHandler() {
  ...
  const isFormValid = aadhaarNumber.replace(/\D/g, "").length === 12 && uidaiConsent;
  ...
  const sendOtp = () => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      setOtpSent(true);
      updateFormData({ aadhaarNumber: aadhaarNumber.replace(/\D/g, ""), ekycUidaiConsent: uidaiConsent });
    }, 800);
  };
              ...
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-sm text-gray-700 leading-relaxed">
                  {journeyConfig.legalTexts.aadhaarConsent ??
                    "I voluntarily authorize State Bank of India to use my Aadhaar details for eKYC authentication and to fetch my KYC details from UIDAI for account opening. I indemnify State Bank of India against losses arising due to any difference in my name on Aadhaar and PAN, if any."}
                </span>
              </label>
