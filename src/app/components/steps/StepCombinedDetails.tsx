...
export default function StepCombinedDetails() {
  ...
  const [usesPrimaryEmailForComms, setUsesPrimaryEmailForComms] = useState<boolean | null>(
    isNtb ? true : (formData.usesPrimaryEmailForComms === undefined ? null : !!formData.usesPrimaryEmailForComms)
  );
  ...
  // Birth & Residence fields (NTB journey — matching HDFC salary journey page layout)
  ...
  const RESIDENCE_TYPES = ["Owned","Rented","Company Provided","Paying Guest","Others"];
  const [countryOfBirth] = useState("India");
  ...
  const handleContinue = useCallback(() => {
    ...
    const nextData: Record<string, any> = {
      email: isNtb ? (formData.email || email) : displayEmail,
      usesPrimaryEmailForComms: isNtb ? true : usesPrimaryEmailForComms,
      communicationEmail: isNtb ? "" : (usesPrimaryEmailForComms ? "" : communicationEmail),
      fatherName,
      motherName,
      maritalStatus,
      spouseName,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Birth & Residence Details — NTB salary journey (matches HDFC reference layout) */}
        {isNtb && (
          <div className="rounded-xl border border-slate-200/80 bg-slate-50/50 p-5 md:p-6">
            <p className="text-sm font-semibold text-slate-800 mb-1 flex items-center gap-2">
              <MapPin className="w-4 h-4 text-slate-400" />
              Birth &amp; Residence Details
            </p>
            <p className="text-xs text-slate-500 mb-4">Add the details required for account processing.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
              ...
              <div>
                ...
                <Select value={stateOfBirth} onValueChange={setStateOfBirth}>
                  <SelectTrigger className={`enterprise-input flex items-center justify-between ${showErrors && !stateOfBirth ? "error" : ""}`}>
                    <SelectValue placeholder="Select state" />
                  </SelectTrigger>
                  <SelectContent className="rounded-[var(--radius-lg)] border-slate-200 shadow-xl p-2 bg-white max-h-60 overflow-y-auto">
                    {INDIAN_STATES.map((s) => (
                      <SelectItem key={s} value={s} className="rounded-[var(--radius)] focus:bg-slate-50 text-sm py-2 px-3">{s}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {showErrors && !stateOfBirth && (
                  <p className="error-text"><AlertCircle className="w-4 h-4" /> Please select your state of birth.</p>
                )}
              </div>
              <div>
                <label className="form-label flex items-center gap-2">
                ...
                <Input
                  ...
                  className={`enterprise-input ${!stateOfBirth ? "bg-gray-100 text-gray-400 cursor-not-allowed" : ""} ${showErrors && !cityOfBirth ? "error" : ""}`}
                  placeholder={stateOfBirth ? "Enter city" : "Select state first"}
                />
                {showErrors && !cityOfBirth && (
                  <p className="error-text"><AlertCircle className="w-4 h-4" /> Please enter your city of birth.</p>
                )}
              </div>
              <div>
                <label className="form-label flex items-center gap-2">
                ...
                <Select value={typeOfResidence} onValueChange={setTypeOfResidence}>
                  ...
                  <SelectContent className="rounded-[var(--radius-lg)] border-slate-200 shadow-xl p-2 bg-white">
                    {RESIDENCE_TYPES.map((r) => (
                      <SelectItem key={r} value={r} className="rounded-[var(--radius)] focus:bg-slate-50 text-sm py-2 px-3">{r}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {showErrors && !typeOfResidence && (
                  <p className="error-text"><AlertCircle className="w-4 h-4" /> Please select your type of residence.</p>
                )}
              </div>
            </div>
          </div>
